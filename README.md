...
Client side is when the web page handles the routing through JavaScript. Whenever there is a request, the app will prevent any request being made to the server. The app will update the state of necessary data. This prevents the entire document being refreshed and the app will be faster than a app with server side routing.
